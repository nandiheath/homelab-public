...
- HLP-021 changed only the repository credential URL in source and generated artifact to `ghcr.io/nandiheath/charts`. Focused rendering, offline repository validation, task lifecycle validation, and exact source/artifact inspection passed. Resource identity, repository-secret labels, `type: helm`, OCI enablement, username/password templates, and the exact unresolved 1Password remote references remain unchanged. No legacy package prefix, private AppProject, wildcard source, image pull secret, resolved credential, bootstrap, cluster, Argo CD, secret-resolution, registry mutation, or deployment operation remains in or was added to current public desired state.
