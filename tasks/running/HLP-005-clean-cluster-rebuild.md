...
- The authorized reset ran each generated K3s server uninstall entrypoint, after which all three hosts stopped accepting TCP connections while still answering ICMP. Residue removal and host-preservation assertions did not run. Recovery now requires restoring SSH reachability without changing host identity; do not start kernel or install phases.

## Completion handoff

- Summary: Completed and live-verified the guarded reset preflight, then began the authorized destructive reset. Delegated host checks use inventory SSH connections, live user identity is compared by host, and embedded-etcd health is read from each server's local metrics endpoint instead of requiring an unshipped `etcdctl`.
...
- Observed verification: Reset syntax and lifecycle entrypoint fixtures passed. The full live reset preflight captured all three preservation baselines, revalidated hostname/user/network identity on every host, observed three unique healthy `v3.5.21` etcd members with one leader and two active peers each, captured API/Application/PVC facts, and passed every operation-guard check. With the exact reset confirmation, all three generated K3s server uninstall entrypoints ran and TCP 6443 became unreachable; the controller kubeconfig was removed. The following privileged residue-removal task could not obtain a fresh sudo credential, and all three hosts now answer ICMP but time out on TCP 22 and 9100. Therefore residue removal and post-reset preservation/absence checks have not completed. `./scripts/validate-ansible.sh`, `./scripts/validate.sh`, the public/private repository graph check, private inventory parsing, and private `make validate` passed before mutation; manifest validation reported `352` resources, `245` valid, `0` invalid, `0` errors, and `107` skipped.
- Follow-ups: Restore SSH reachability on all three hosts, then use a guarded recovery path with an available become credential to finish residue removal and preservation/absence checks. Do not start kernel activation, install, network bootstrap, or GitOps until reset acceptance is complete; each later phase remains separately gated.
