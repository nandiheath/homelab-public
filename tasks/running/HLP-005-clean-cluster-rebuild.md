...
- Live reset, install, cluster, router, and remote-host mutations require explicit current-session authorization. The 2026-08-09 inspection was explicitly read-only; it changed no cluster resource.

## Blockers

- The 2026-08-09 live GitOps bootstrap is incomplete. An offline repair now removes ambient enrollment from the Argo CD and Istio control planes, installs and verifies `istiod`, Istio CNI, and ztunnel before Argo CD, restarts the stranded Argo workloads, and waits for every expected child Application and Hubble TLS. The revised source must be reviewed and published before a fresh exact live-recovery authorization; no live repair has run.
...
- Offline bootstrap-repair evidence: `scripts/bootstrap.sh` now has a credential-free dry-run, exact `BOOTSTRAP <cluster-id>` guard, protected kubeconfig preflight, staged Istio-before-Argo application and rollout gates, failed-probe recovery, public/private root ownership handoff, every-child Application health gates, Hubble TLS checks, and final non-ambient assertions for `argocd` and `istio-system`. The namespace source removes both ambient labels. `tests/bootstrap/run.sh` executes the guarded flow against a fake kubectl, proves the mutation order and label removal, and rejects wrong authorization. No live cluster operation was performed.
- Remaining live boundary: publish the reviewed public revision so Argo cannot restore the obsolete ambient labels, obtain a fresh exact bootstrap authorization, run the repaired script, and complete the PVC, smoke-test, and residue acceptance checks. Do not run the local fix against a public root still tracking the old revision.
- Final offline repair validation: public `make validate` passed after the staged script and namespace render (`51` Ansible files linted with `0` failures/warnings; lifecycle fixtures passed; bootstrap fixtures passed; `349` resources, `245` valid, `0` invalid, `0` errors, `104` schema-skipped). The earlier Python `SyntaxWarning` did not recur. Private `make validate` and `./scripts/validate-repository-graph.sh ../homelab-private` also passed. No kubeconfig-backed command or live infrastructure mutation was executed.
