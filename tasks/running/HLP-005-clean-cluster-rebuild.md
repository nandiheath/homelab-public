...
- Remaining live boundary: publish both reviewed revisions so the renamed roots and corrected control-plane namespace state are available, then run the revised bootstrap script and allow Argo CD to reconcile child Applications. Complete PVC, workload, smoke-test, and residue acceptance separately. No live recovery was authorized or run by this source change.
