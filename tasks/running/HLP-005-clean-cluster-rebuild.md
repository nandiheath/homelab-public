...
- Final acceptance proves node/API/Cilium health, expected GitOps health, exactly three Bound PVCs—the two reviewed Immich claims and the standalone Redis claim—empty smoke tests, no token/credential residue, and no backup policy.
...
- Remaining live boundary: publish the private removal of the obsolete Prometheus VirtualService, allow Argo CD to prune that dead route, and repeat the final read-only route and residue checks. The two operator-supplied Connect input files remain on the controller at corrected mode `0600`; they are durable protected inputs, not bootstrap-temporary credential files.
